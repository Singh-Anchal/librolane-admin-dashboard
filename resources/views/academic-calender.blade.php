    ...
    <div class="min-h-screen bg-slate-50 dark:bg-[#101922] p-6 lg:p-8 font-sans text-slate-900 dark:text-white">
        ...
        <div class="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-8">
            <div>
                ...
                <p class="text-sm text-slate-500 dark:text-slate-400 mt-1">Manage school events, holidays, and exam schedules</p>
            </div>
            
            <div class="flex items-center gap-3">
                {{-- Year Dropdown --}}
                <div class="relative">
                    <select class="appearance-none bg-white dark:bg-[#18232f] border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-200 py-2 pl-4 pr-10 rounded-lg text-sm font-medium focus:outline-none focus:ring-2 focus:ring-blue-500 shadow-sm cursor-pointer">
                ...
                {{-- Add Event Button --}}
                <button class="flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg text-sm font-bold shadow-sm transition-colors">
        ...
        <div class="grid grid-cols-1 xl:grid-cols-4 gap-6">
            ...
            <div class="xl:col-span-3 bg-white dark:bg-[#18232f] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                
                {{-- Calendar Header --}}
                <div class="flex items-center justify-between mb-6">
                    <h2 class="text-xl font-bold text-slate-900 dark:text-white">{{ $monthName }} {{ $year }}</h2>
                ...
                <div class="grid grid-cols-7 mb-4">
                    @foreach(['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'] as $day)
                        <div class="text-center text-sm font-medium text-slate-400 dark:text-slate-500">{{ $day }}</div>
                    @endforeach
                </div>

                {{-- Calendar Grid Cells --}}
                <div class="grid grid-cols-7 gap-px bg-slate-100 dark:bg-slate-800 border border-slate-100 dark:border-slate-800 rounded-xl overflow-hidden">
                    ...
                    @for($day = 1; $day <= $totalDays; $day++)
                        ...
                        <div class="relative bg-white dark:bg-[#18232f] min-h-[120px] p-3 transition-colors hover:bg-slate-50 dark:hover:bg-slate-800/50 group cursor-pointer 
                            {{ $isSelected ? 'ring-1 ring-inset ring-emerald-500 bg-emerald-50/10 z-10' : '' }}">
                            
                            {{-- Day Number --}}
                            <span class="text-sm font-semibold {{ $isSelected ? 'text-emerald-600 dark:text-emerald-400' : 'text-slate-700 dark:text-slate-300' }}">
                                {{ $day }}
                            </span>
                            ...
                            <div class="absolute bottom-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                                <div class="w-6 h-6 rounded-full bg-slate-100 dark:bg-slate-700 flex items-center justify-center text-slate-400 hover:text-blue-600">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                                </div>
                            </div>
                        </div>
            ...
            <div class="flex flex-col gap-6">
                ...
                <div class="bg-white dark:bg-[#18232f] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                    <h3 class="font-bold text-slate-900 dark:text-white mb-4">Select a Date</h3>
                    <div class="flex flex-col items-center justify-center py-6 text-center">
                        <div class="w-12 h-12 bg-slate-50 dark:bg-slate-800 rounded-full flex items-center justify-center text-slate-400 mb-3">
                ...
                <div class="bg-white dark:bg-[#18232f] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                    ...
                    @if(count($events) > 0)
                        <div class="space-y-4">
                            @foreach($events as $event)
                                <div class="flex items-start gap-3 pb-3 border-b border-slate-50 dark:border-slate-800 last:border-0 last:pb-0">
                                    <div class="flex flex-col items-center justify-center w-10 h-10 rounded-lg bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700 shrink-0">
                                        <span class="text-xs font-bold text-slate-500 dark:text-slate-400">Jan</span>
                                        <span class="text-sm font-bold text-slate-900 dark:text-white">{{ $event['day'] }}</span>
                                    </div>
                                    <div>
                ...
                <div class="bg-white dark:bg-[#18232f] border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-6">
                    ...
                    <div class="flex items-center justify-between mb-4">
                        <button class="p-1 hover:bg-slate-100 rounded text-slate-400"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15 19l-7-7 7-7"></path></svg></button>
                        <span class="text-sm font-bold text-slate-900 dark:text-white">{{ $monthName }} {{ $year }}</span>
                        <button class="p-1 hover:bg-slate-100 rounded text-slate-400"><svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"></path></svg></button>
                    </div>
                    ...
                    <div class="grid grid-cols-7 text-center gap-y-2">
                        ...
                        @for($day = 1; $day <= $totalDays; $day++)
                            ...
                            <div class="flex items-center justify-center">
                                <span class="w-7 h-7 flex items-center justify-center rounded-md text-sm cursor-pointer transition-colors
                                    {{ $isSelected 
