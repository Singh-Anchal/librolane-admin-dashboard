...
<body class="font-display bg-v-bg text-[#495057] antialiased">
    ...
    <script>
        ...
        function showToast(message, type = 'success') {
            ...
            toast.className =
                `pointer-events-auto flex w-full max-w-xs items-center gap-3 rounded-md ${config.bg} p-4 shadow-lg ${config.border} toast-enter`;
            toast.innerHTML =
                `<span class="material-symbols-outlined ${config.text}">${config.icon}</span><div class="flex-1"><p class="text-sm font-medium text-gray-700">${message}</p></div><button onclick="this.parentElement.remove()" class="text-gray-400 hover:text-gray-600"><span class="material-symbols-outlined text-[18px]">close</span></button>`;
